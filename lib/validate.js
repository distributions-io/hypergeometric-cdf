...
* @param {Number} [options.m=1] - number of white balls in urn
* @param {Number} [options.n=1] - number of black balls in urn
* @param {Number} [options.k=1] - number of draws
...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'm' ) ) {
		...
		if ( !isNonNegativeInteger( opts.m ) ) {
			return new TypeError( 'cdf()::invalid option. `m` parameter must be a non-negative integer. Option: `' + opts.m + '`.' );
		}
	}
